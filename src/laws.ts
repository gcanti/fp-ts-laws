...
export const setoid = {
  reflexivity: <A>(S: Setoid<A>) => (a: A): boolean => {
    return S.equals(a, a)
  },
  simmetry: <A>(S: Setoid<A>) => (a: A, b: A): boolean => {
    return S.equals(a, b) === S.equals(b, a)
  },
  transitivity: <A>(S: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return (S.equals(a, b) && S.equals(b, c)) === (S.equals(a, b) && S.equals(a, c))
  }
}
...
export const semigroup = {
  associativity: <A>(S: Semigroup<A>, Eq: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return Eq.equals(S.concat(S.concat(a, b), c), S.concat(a, S.concat(b, c)))
  }
}

export const monoid = {
  rightIdentity: <A>(M: Monoid<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return Eq.equals(M.concat(a, M.empty), a)
  },
  leftIdentity: <A>(M: Monoid<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return Eq.equals(M.concat(M.empty, a), a)
  }
}
...
export const semiring = {
  addAssociativity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return Eq.equals(S.add(S.add(a, b), c), S.add(a, S.add(b, c)))
  },
  addIdentity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return allEquals(Eq)(a, S.add(a, S.zero), S.add(S.zero, a))
  },
  commutativity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A, b: A): boolean => {
    return Eq.equals(S.add(a, b), S.add(b, a))
  },
  mulAssociativity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return Eq.equals(S.mul(S.mul(a, b), c), S.mul(a, S.mul(b, c)))
  },
  mulIdentity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return allEquals(Eq)(a, S.mul(a, S.one), S.mul(S.one, a))
  },
  leftDistributivity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return Eq.equals(S.mul(a, S.add(b, c)), S.add(S.mul(a, b), S.mul(a, c)))
  },
  rightDistributivity: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A, b: A, c: A): boolean => {
    return Eq.equals(S.mul(S.add(a, b), c), S.add(S.mul(a, c), S.mul(b, c)))
  },
  annihilation: <A>(S: Semiring<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return allEquals(Eq)(S.zero, S.mul(a, S.zero), S.mul(S.zero, a))
  }
}

export const ring = {
  additiveInverse: <A>(R: Ring<A>, Eq: Setoid<A>) => (a: A): boolean => {
    return allEquals(Eq)(R.sub(a, a), R.add(R.sub(R.zero, a), a), R.zero)
  }
}

export const field = {
  commutativity: <A>(F: Field<A>, S: Setoid<A>) => (a: A, b: A): boolean => {
    return S.equals(F.mul(a, b), F.mul(b, a))
  },
  integralDomain: <A>(F: Field<A>, S: Setoid<A>) => (a: A, b: A): boolean => {
    return S.equals(a, F.zero) || S.equals(b, F.zero) || !S.equals(F.mul(a, b), F.zero)
  },
  nonNegativity: <A>(F: Field<A>, S: Setoid<A>) => (a: A): boolean => {
    return S.equals(a, F.zero) || F.degree(a) >= 0
  },
  quotient: <A>(F: Field<A>, S: Setoid<A>) => (a: A, b: A): boolean => {
    const q = F.div(a, b)
    const r = F.mod(a, b)
    return S.equals(b, F.zero) || S.equals(a, F.add(F.mul(q, b), r))
  },
  reminder: <A>(F: Field<A>, S: Setoid<A>) => (a: A, b: A): boolean => {
    const r = F.mod(a, b)
    return S.equals(b, F.zero) || S.equals(r, F.zero) || F.degree(a) <= F.degree(b)
  },
  submultiplicative: <A>(F: Field<A>, S: Setoid<A>) => (a: A, b: A): boolean => {
    return S.equals(a, F.zero) || S.equals(b, F.zero) || F.degree(a) <= F.degree(F.mul(a, b))
  },
  inverse: <A>(F: Field<A>, S: Setoid<A>) => (a: A): boolean => {
