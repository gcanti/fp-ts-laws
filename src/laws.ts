...
export const functor = {
  ...
  composition: <F, A, C>(F: Functor<F>, S: Setoid<HKT<F, C>>) => <B>(
    fa: HKT<F, A>,
    ab: Function1<A, B>,
    bc: Function1<B, C>
...
export const applicative = {
  ...
  homomorphism: <F, A, B>(F: Applicative<F>, S: Setoid<HKT<F, B>>) => (a: A, ab: Function1<A, B>) =>
    S.equals(F.ap(F.of(ab), F.of(a)), F.of(ab(a))),
  interchange: <F, A, B>(F: Applicative<F>, S: Setoid<HKT<F, B>>) => (fab: HKT<F, Function1<A, B>>, a: A) =>
    S.equals(F.ap(fab, F.of(a)), F.ap(F.of((ab: Function1<A, B>) => ab(a)), fab)),
  derivedMap: <F, A, B>(F: Applicative<F>, S: Setoid<HKT<F, B>>) => (fa: HKT<F, A>, ab: Function1<A, B>) =>
    S.equals(F.map(fa, ab), F.ap(F.of(ab), fa))
}

export const chain = {
  associativity: <F, A, B, C>(F: Chain<F>, S: Setoid<HKT<F, C>>) => (
    fa: HKT<F, A>,
    afb: Function1<A, HKT<F, B>>,
    bfc: Function1<B, HKT<F, C>>
  ) => S.equals(F.chain(F.chain(fa, afb), bfc), F.chain(fa, a => F.chain(afb(a), bfc))),
  derivedAp: <F, A, B>(F: Chain<F>, S: Setoid<HKT<F, B>>) => (fa: HKT<F, A>, fab: HKT<F, Function1<A, B>>) =>
    S.equals(F.ap(fab, fa), F.chain(fab, f => F.map(fa, f)))
}

export const monad = {
  leftIdentity: <F, A, B>(F: Monad<F>, S: Setoid<HKT<F, B>>) => (a: A, afb: Function1<A, HKT<F, B>>) =>
    S.equals(F.chain(F.of(a), afb), afb(a)),
  rightIdentity: <F, A>(F: Monad<F>, S: Setoid<HKT<F, A>>) => (fa: HKT<F, A>) => S.equals(F.chain(fa, F.of), fa),
  derivedMap: <F, A, B>(F: Monad<F>, S: Setoid<HKT<F, B>>) => (fa: HKT<F, A>, ab: Function1<A, B>) =>
